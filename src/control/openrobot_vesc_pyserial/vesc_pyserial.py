...
class VESC_USB:
    ...
    def get_controller_id_list(self):
        ...
        self.volt_data = [[np.nan, np.nan]]*len(self.controller_id_list)
        ...
        if start_byte == 2 and end_byte == 3 and crc_result:
            ...
            if command == COMM_PACKET_ID['COMM_FW_VERSION']:
                ...
                for i in range(can_devs_num):
                    ...
                    controller_id = data_frame[ind_f]; ind_f += 1
                    # volt_input = self.get_bytes(data_frame[ind_f:ind_f+2], 10); ind_f += 2
                    # self.volt_data.append([controller_id, round(volt_input,3)])
                    # self.volt_data.sort()
                    temp_motor = self.get_bytes(data_frame[ind_f:ind_f+2], 10); ind_f += 2
                    ...
                    self.pos_data.append([controller_id, round(accum_pos_now,2)])
                    self.pos_data.sort()
                    rps = self.get_bytes(data_frame[ind_f:ind_f+2], 100); ind_f += 2
    ...
    def get_imu_status(self):
        return self.imu_status_data
    
    def get_imu_cmg(self):
        return self.imu_cmg_data
    
    # def get_input_volt(self):
    #     return self.values.volt_input
